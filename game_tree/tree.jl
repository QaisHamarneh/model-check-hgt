...
function count_nodes(root::Node, level::Int = 0)::Int
    ...
    @match root begin
        ...
        Node(_, _, _, _, _, []) => 1
        Node(_, _, _, _, _, children) => 1 + sum(count_nodes(child, level + 1) for child in children)
    end
end
...
function depth_of_tree(root::Node, level::Int = 1)::Int
    @match root begin
        ...
        Node(_, _, _, _, _, children) => maximum(depth_of_tree(child, level + 1) for child in children)
    end
end
