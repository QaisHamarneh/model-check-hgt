...
function run_discrete_test()
    ...
    invariant1(valuation) = valuation["x"] < 40
    invariant2(valuation) = valuation["y"] < 40
    invariant3(valuation) = valuation["y"] < 40 && valuation["x"] < 40

    guard1(valuation) = valuation["x"] < 50
    guard2(valuation) = valuation["y"] < 50
    ...
    one_agent::Vector{Unsigned} = [1] 

    actions = ["a", "b"]

    e1 = Edge("e1", l1, l2, guard1, Dict(1 => "a"), Dict("x" => 1))
    e2 = Edge("e2", l2, l1, guard2, Dict(1 => "b"), Dict("y" => 1))



    two_agents::Vector{Unsigned} = [1, 2]

    e12 = Edge("e12", l1, l2, guard1, Dict(1 => "a", 2 => "a"), Dict("x" => 1))
    e13 = Edge("e13", l1, l3, guard2, Dict(1 => "b", 2 => "a"), Dict("y" => 1))

    e21 = Edge("e21", l2, l1, guard1, Dict(2 => "a", 1 => "a"), Dict("x" => 1))
    e23 = Edge("e23", l2, l3, guard2, Dict(2 => "b", 1 => "a"), Dict("y" => 1))

    e31 = Edge("e31", l3, l1, guard1, Dict(1 => "a", 2 => "b"), Dict("x" => 1))
    e32 = Edge("e32", l3, l2, guard2, Dict(1 => "b", 2 => "b"), Dict("y" => 1))
    ...
    game_tree = build_game_tree(small_game, 5.0)


    println("*************************")
    println(count_nodes(game_tree))
    println("*************************")
